...
public class Canvas {
    ...
    public static final Canvas getCanvas() {
        ...
        // Initialize CANVAS if it is null
        if (CANVAS == null) {
            try {
                CANVAS = new Canvas(600, 600);
            } catch (IOException e) {
                System.out.println("Unable to load image files. Are you sure the turtle image files are still there?");
            }
        }
        // Return the CANVAS
        return CANVAS;
...
/**
 * ActorPanel class. Extends the JPanel component to make an AWT component that
 * draws Actors.
 * 
 * @author (Nikolaj Ignatieff Schwartzbach).
 */
class ActorPanel extends JPanel {
    ...
    // ---------------------\\
    /**
     * Create a new ActorPanel. An ActorPanel draws the Actors (turtles), which have
     * been added to this ActorPanel.
     * 
    ...
    /**
     * Clears the content of this ActorPanel (removes all lines the Actors have
     * drawn).
     */
    public void clear() {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Actor a : actors) {
            ...
            trans.rotate(Math.toRadians(a.getAngle()), a.getX(), a.getY());
            // Apply it to the graphics
            g2d.transform(trans);

            // Draw the turtle with center at (x,y)
            g2d.drawImage(imgTurtle, (int) a.getX() - imgTurtle.getWidth() / 2,
                    (int) a.getY() - imgTurtle.getHeight() / 2, null);
            ...
            if (a.isPenDown()) {
                ...
                // Draw circle
                g2d.fill(new Ellipse2D.Double((int) a.getX() - CIRCLE_RADIUS, (int) a.getY() - CIRCLE_RADIUS,
